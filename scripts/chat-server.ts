...
function enforceVykanie(value: string): string {
  return value
    .replace(/napíštete/g, "napíšte")
    .replace(/Napíštete/g, "Napíšte")
    .replace(/pošlitete/g, "pošlite")
    .replace(/Pošlitete/g, "Pošlite")
    .replace(/si testuješ/g, "testujete")
    .replace(/Si testuješ/g, "Testujete")
    ...
    .replace(/\bmáš\b/g, "máte")
    .replace(/\bMas\b/g, "Máte")
    .replace(/\bmas\b/g, "máte")
    .replace(/\bChceš\b/g, "Chcete")
    .replace(/\bchceš\b/g, "chcete")
    .replace(/\bChces\b/g, "Chcete")
    .replace(/\bchces\b/g, "chcete")
    .replace(/\bVieš\b/g, "Viete")
    .replace(/\bvieš\b/g, "viete")
    .replace(/\bVies\b/g, "Viete")
    .replace(/\bvies\b/g, "viete")
    .replace(/\bNevieš\b/g, "Neviete")
    .replace(/\bnevieš\b/g, "neviete")
    .replace(/\bNevies\b/g, "Neviete")
    .replace(/\bnevies\b/g, "neviete")
    .replace(/\bRiešiš\b/g, "Riešite")
    .replace(/\briešiš\b/g, "riešite")
    .replace(/\bRiesis\b/g, "Riešite")
    .replace(/\briesis\b/g, "riešite")
    .replace(/\bUvažuješ\b/g, "Uvažujete")
    .replace(/\buvažuješ\b/g, "uvažujete")
    .replace(/\bZvažuješ\b/g, "Zvažujete")
    .replace(/\bzvažuješ\b/g, "zvažujete")
    .replace(/\bPotrebuješ\b/g, "Potrebujete")
    .replace(/\bpotrebuješ\b/g, "potrebujete")
    .replace(/\bMôžeš\b/g, "Môžete")
    .replace(/\bmôžeš\b/g, "môžete")
    .replace(/\bMozes\b/g, "Môžete")
    .replace(/\bmozes\b/g, "môžete")
    .replace(/\bPošli\b/g, "Pošlite")
    .replace(/\bpošli\b/g, "pošlite")
    .replace(/\bPosli\b/g, "Pošlite")
    .replace(/\bposli\b/g, "pošlite")
    ...
    .replace(/\bdoplň\b/g, "doplňte")
    .replace(/\bDoplňte mi\b/g, "Doplňte prosím")
    .replace(/\bdoplňte mi\b/g, "doplňte prosím")
    ...
    .replace(/\bpre Vás\b/g, "pre vás")
    .replace(/\bpri Vašom\b/g, "pri vašom")
    .replace(/\bvo Vašom\b/g, "vo vašom")
    .replace(/napíštete/g, "napíšte")
    .replace(/Napíštete/g, "Napíšte")
    .replace(/pošlitete/g, "pošlite")
    .replace(/Pošlitete/g, "Pošlite");
...
function enforceStructuredAnswer(answer: StructuredAnswer, sources: ChatSource[], mode: AnswerMode): StructuredAnswer {
  ...
  if (mode === "general_chat") {
    const shortAnswer = statementText(enforceVykanie(answer.shortAnswer.replace(/\bHVAC\b/gi, "vykurovanie a chladenie")), 220);
    const details = answer.details
      .map((detail) => enforceVykanie(statementText(detail.replace(/\bHVAC\b/gi, "vykurovanie a chladenie"), 180)))
      .filter(Boolean)
      .slice(0, 2);
    const followUp = firstFollowUp(enforceVykanie((answer.followUpQuestion || "").replace(/\bHVAC\b/gi, "vykurovanie alebo chladenie")));
    return {
  ...
  const details = answer.details
    ...
    .map((detail) => enforceVykanie(statementText(detail, 220)))
    .filter(Boolean)
  ...
  return {
    shortAnswer: enforceVykanie(statementText(answer.shortAnswer, 340)),
    details,
    followUpQuestion: followUpQuestion ? enforceVykanie(followUpQuestion) : null,
    shouldAskFollowUp: Boolean(answer.shouldAskFollowUp && followUpQuestion),
    safetyNote: answer.safetyNote ? enforceVykanie(statementText(answer.safetyNote, 220)) : null,
    confidence,
  };
}
...
function serviceLabel(serviceType: ServiceType): string {
  switch (serviceType) {
    ...
    case "screeds":
      return "potery";
    case "company":
      return "firemné a obchodné informácie";
    case "service":
...
function quoteHandoffTopicSentence(message: string, state: QualificationState, route: Pick<ServiceRoute, "serviceType" | "serviceIntent">): string {
  const text = normalizePolicyText([message, state.service_type || "", state.service_intent || "", route.serviceType, route.serviceIntent].join(" "));
  const service = normalizeServiceType(state.service_type || route.serviceType, "unknown");
  if (/(video|navod|návod|navody|návody)/.test(text) && /(tepelne cerpad|tepelné čerpad|cerpad|čerpad|rekuper|zariaden|model)/.test(text)) {
    return "Dopyt sa týka návodov k zariadeniam, takže konzultácia má riešiť konkrétny model, zariadenie, servis alebo údržbu.";
  }
  if (/(drazic|dražic|argo)/.test(text)) {
    return "Dopyt sa týka riešenia Dražice ARGO v oblasti tepelného čerpadla alebo vykurovania, takže treba potvrdiť aktuálnu dostupnosť, vhodnosť a montáž.";
  }
  if (/(plan obnovy|plán obnovy|dotac|dotác|poukaz|prispev|príspev|podpor)/.test(text)) {
    return "Dopyt sa týka dotácie alebo podpory, takže treba preveriť vhodné riešenie, podmienky programu a nadväzujúce nacenenie.";
  }
  if (/(rekuper|vetran|vzduchotechn)/.test(text) || (/\bvzduch\b/.test(text) && !/(tepelne cerpad|tepelné čerpad|cerpad|čerpad|vzduch voda|vzduch-voda)/.test(text))) {
    return "Dopyt sa týka rekuperácie, vetrania alebo vzduchu v budove, takže nacenenie má riešiť jednotku, rozvody, dispozíciu a technickú miestnosť.";
  }
  ...
  if (/(bkt|betonove jadro|betónové jadro|temperovanie)/.test(text)) {
    return "Dopyt sa týka temperovania betónového jadra, stropného alebo stenového chladenia a regulácie, takže nacenenie má riešiť plochy, zdroj chladu a rosný bod.";
  }
  if (/(velke stavby|veľké stavby|polyfunkc|referenc)/.test(text)) {
    return "Dopyt sa týka väčšej stavby alebo referencie, takže nacenenie má riešiť technické riešenia vykurovania, vetrania alebo chladenia podľa objektu.";
  }
  if (/(ocenen|certifikat|certifikát|certifikaty|certifikáty)/.test(text)) {
    return "Dopyt sa týka Geotherm, ocenení, certifikátov alebo referencií, takže odpoveď má potvrdiť firemné zázemie a až potom riešiť konkrétnu realizáciu.";
  }
  if (/(tzb|navrh vykurov|návrh vykurov|vykurovania|vykurovanie|kurenie|kúrenie|kureni|návrh a cenová ponuka)/.test(text)) {
    return "Dopyt sa týka návrhu vykurovania alebo technického riešenia budovy, takže nacenenie má riešiť zdroj tepla, rozvody, reguláciu a rozsah prác.";
  }
  if (service === "radiators" || /(radiator|radiátor|radiatory|radiátory)/.test(text)) {
    return "Dopyt sa týka radiátorov alebo vykurovacích telies, takže nacenenie má riešiť typ telies, teplotný spád, rozvody a zdroj tepla.";
  }
  if (service === "heat_pump" || /(tepelne cerpad|tepelné čerpad|cerpad|čerpad|\btc\b|\btč\b)/.test(text)) {
    return "Dopyt sa týka tepelného čerpadla a vykurovania, takže nacenenie má riešiť typ systému, výkon, montáž, TÚV, reguláciu a rozsah kotolne.";
  }
  if (service === "floor_heating" || /podlahov|podlahove kurenie|podlahové kúrenie/.test(text)) {
    return "Dopyt sa týka podlahového kúrenia, takže nacenenie má riešiť plochu, skladbu podlahy, okruhy, rozdeľovač a napojenie na zdroj tepla.";
  }
  if (service === "solar_photovoltaic" || /(fotovolt|solar|solarn|solár|panel|kolektor|zasobnik|zásobnik)/.test(text)) {
    return "Dopyt sa týka fotovoltaiky alebo solárneho riešenia, takže nacenenie má riešiť panely alebo kolektory, výkon, strechu, zásobník a napojenie.";
  }
  if (service === "heat_recovery" || /(rekuper|vetran|vzduchotechn)/.test(text) || (/\bvzduch\b/.test(text) && !/(tepelne cerpad|tepelné čerpad|cerpad|čerpad|vzduch voda|vzduch-voda)/.test(text))) {
    return "Dopyt sa týka rekuperácie a vetrania, takže nacenenie má riešiť jednotku, rozvody vzduchu, dispozíciu a technickú miestnosť.";
  }
  if (service === "air_conditioning" || /(klimatiz|klima|chladen)/.test(text)) {
    return "Dopyt sa týka klimatizácie alebo chladenia, takže nacenenie má riešiť počet miestností, výkon, vonkajšiu jednotku a montáž.";
  }
  if (service === "water" || /(rozvod.*vod|uprava vody|úprava vody|zmakcovac|zmäkčovač|kanaliz)/.test(text)) {
    return "Dopyt sa týka vody alebo rozvodov, takže nacenenie má riešiť rozsah prác, pripojenia, materiál a stav existujúcich rozvodov.";
  }
  if (service === "screeds" || /poter/.test(text)) {
    return "Dopyt sa týka poterov, takže nacenenie má riešiť plochu, typ poteru, hrúbku, podklad a väzbu na podlahové kúrenie.";
  }
  if (service === "central_vacuum" || /(centraln.*vysav|vysavac|vysávač)/.test(text)) {
    return "Dopyt sa týka centrálneho vysávača, takže nacenenie má riešiť trasu rozvodov, počet zásuviek, jednotku a dispozíciu.";
  }
  if (service === "sanitary" || /(geberit|zdravotechn|sanit|\bwc\b)/.test(text)) {
    return "Dopyt sa týka zdravotechniky alebo sanity, takže nacenenie má riešiť rozsah rozvodov, napojenia, WC alebo sanitárne prvky.";
  }
  if (service === "boilers" || /(kotol|kotly|plynov)/.test(text)) {
    return "Dopyt sa týka kotla alebo kotolne, takže nacenenie má riešiť typ zdroja, odvod spalín, reguláciu, montáž a potrebné podklady.";
  }
  if (service === "ceiling_cooling" || /(stropn|stenn|rosny bod|rosný bod)/.test(text)) {
    return "Dopyt sa týka stropného alebo stenového chladenia, takže nacenenie má riešiť plochy, reguláciu vlhkosti, rosný bod a zdroj chladu.";
  }
  if (service === "service" || /(servis|poruch|udrzb|údržb|chybovy|chybový)/.test(text)) {
    return "Dopyt sa týka servisu alebo údržby, takže treba preveriť zariadenie, značku/model, prejav problému, lokalitu a dostupnosť technika.";
  }
  return "Dopyt treba naceniť podľa konkrétnej služby, rozsahu prác, lokality a dostupných podkladov.";
}

function quoteTopicRepairSentence(message: string, answer: string): string | null {
  const text = normalizePolicyText(message);
  const normalizedAnswer = normalizePolicyText(answer);
  const missing = (pattern: RegExp): boolean => !pattern.test(normalizedAnswer);
  if (/(drazic|dražic|argo)/.test(text) && missing(/tepel|cerpad|vykurov/)) {
    return "Pri Dražice ARGO to beriem ako tému tepelného čerpadla alebo vykurovania, takže nacenenie má riešiť vhodnosť zdroja tepla, montáž a aktuálnu ponuku.";
  }
  if (/(plan obnovy|plán obnovy|dotac|dotác|poukaz|prispev|príspev|podpor)/.test(text) && missing(/dot|podpor|over|podmien/)) {
    return "Pri Pláne obnovy alebo dotáciách treba preveriť podporu, podmienky programu, oprávnenosť a nadväzujúce nacenenie riešenia.";
  }
  if ((/(rekuper|vetran|vzduchotechn)/.test(text) || (/\bvzduch\b/.test(text) && !/(tepelne cerpad|tepelné čerpad|cerpad|čerpad|vzduch voda|vzduch-voda)/.test(text))) && missing(/rekuper|vetr|vzduch/)) {
    return "Téma sa týka rekuperácie, vetrania alebo vzduchu v budove, preto má ponuka riešiť jednotku, rozvody, dispozíciu a technickú miestnosť.";
  }
  ...
  if (/(navrh vykurov|návrh vykurov|vykurovania|vykurovanie|kurenie|kúrenie|tzb)/.test(text) && missing(/kuren|vykurov|navrh|návrh|riesenie|riešenie/)) {
    return "Téma je návrh vykurovania alebo technické riešenie budovy, preto má ponuka riešiť zdroj tepla, rozvody, reguláciu a rozsah prác.";
  }
  if (/(velke stavby|veľké stavby|polyfunkc)/.test(text) && missing(/vykurov|vetr|chladen|refer/)) {
    return "Pri veľkých stavbách a polyfunkčných budovách ide najmä o referencie a technické riešenia vykurovania, vetrania alebo chladenia podľa objektu.";
  }
  if (/(ocenen|certifikat|certifikát|certifikaty|certifikáty)/.test(text) && missing(/geotherm|realiz|refer|certifik/)) {
    return "Téma sa týka Geotherm, realizácií, referencií, ocenení alebo certifikátov, preto ju netreba zamieňať za výber konkrétneho zariadenia.";
  }
  if (/(radiator|radiátor|radiatory|radiátory)/.test(text) && missing(/radi|vykurov|teplot/)) {
    return "Pri radiátoroch treba riešiť vykurovacie telesá, teplotný spád, rozvody a zdroj tepla.";
  }
  if (/(bkt|betonove jadro|betónové jadro|temperovanie)/.test(text) && missing(/chladen|strop|sten|regul/)) {
    return "Temperovanie betónového jadra BKT súvisí so stropným alebo stenovým chladením a reguláciou, preto treba riešiť aj vlhkosť a rosný bod.";
  }
  if (/(video|navod|návod|navody|návody)/.test(text) && missing(/servis|udrz|údrž|model|zariaden/)) {
    return "Pri video návodoch ide skôr o konkrétny model, zariadenie, servis alebo údržbu, nie o samostatný výber technológie.";
  }
  return null;
}
...
function serviceSearchKeyword(serviceType: ServiceType): string {
  switch (serviceType) {
    ...
    case "company":
      return "company-truth Geotherm kontakt posobnost referencie platby dokumentacia obchodny postup";
    case "service":
    ...
    slotOnlyReply
      ...
      : /(dotac|subsidy|poukazk|prispevok|zelen[ae] domacnost)/.test(text)
        ...
        : /(poter|potery|anhydrit|cementov)/.test(text)
          ? "screeds"
        : /(posobnost|pôsobnosť|okres|mesto|sluzb|služb|pre firmy|male zakazky|malé zákazky|poisten|referenc|recenzi|ukazk|realizac|material|materiál|platb|faktur|zaloha|záloha|splatk|splátk|whatsapp|email|e-mail|telefon|telefonicky|kontakt|dokumentac|showroom|roky ste na trhu|kontakt pocas realizacie|kontakt počas realizácie|nacen.*projekt|nacen.*fot|ponuk.*projekt|ponuk.*fot)/.test(text)
          ? "company"
...
function serviceCardSummary(serviceType: ServiceType): string {
  ...
  const cards: Record<ServiceType, string> = {
    ...
    company:
      "Service card firemné a obchodné informácie: použi pri otázkach na pôsobnosť, okresy, kontakty, WhatsApp/e-mail/telefón, referencie, recenzie, showroom, materiál, platby, zálohy, faktúry, dokumentáciu, obchodný postup, malé zákazky a všeobecný rozsah firmy. Odpovedaj opatrne podľa potvrdených podkladov, bez garantovania termínov, cien alebo bezplatnosti.",
    service:
...
function deterministicQualificationUpdate(message: string, route?: Pick<ServiceRoute, "serviceType" | "serviceIntent">): QualificationUpdate {
  ...
  const areaMatch = normalized.match(/(\d{2,4})\s*(?:m2|m 2|m2|m\b|metrov|metre)/) || normalized.match(/\b(\d{2,4})\b/);
  if (areaMatch) {
...
function expectedVerdictAnswer(state: QualificationState): string {
  if (isNewBuildFloorHeating(state)) {
    const area = state.area_m2 ? ` pri dome cca ${state.area_m2} m2` : "";
    const parts = [
      "### Predbežný verdikt",
      "",
...
function expectedAirConditioningAnswer(): string {
  return [
    ...
    "",
    "Koľko m2 má obývačka a spálňa a kde by mohla byť vonkajšia jednotka?",
  ].join("\n");
}
...
function buildLeadSummary(input: {
  ...
  const facts = [
    ...
    input.state.area_m2 ? `Plocha približne ${input.state.area_m2} m2.` : null,
...
function companyPracticalDirectAnswer(message: string): DirectAnswerDecision | null {
  ...
  if (
    /(tepelne cerpad|tepelné čerpad|cerpad|čerpad|\btc\b|\btč\b).*(zivotnost|životnosť|spolahliv|spoľahliv|kaziv|poruchov)|(?:zivotnost|životnosť|spolahliv|spoľahliv|kaziv|poruchov).*(tepelne cerpad|tepelné čerpad|cerpad|čerpad|\btc\b|\btč\b)/.test(text)
  ) {
    return answerBase(
      "Životnosť a spoľahlivosť tepelného čerpadla",
      "Spoľahlivosť tepelného čerpadla stojí hlavne na správnom návrhu, odbornej montáži, nastavení regulácie a pravidelnom servise. Bez konkrétneho modelu by som negarantoval presnú životnosť, ale pri kvalitnej zostave a údržbe má zmysel riešiť tepelné čerpadlo ako dlhodobý zdroj vykurovania. Praktický ďalší krok je konzultácia s Geotherm: vybrať vhodnú značku a zostavu podľa domu a nastaviť servisný plán.",
      "heat_pump_lifetime_reliability_scope",
      "process",
      "service-card-heat-pump zivotnost spolahlivost tepelne cerpadlo servis udrzba montaz Geotherm",
    );
  }
  ...
  if (/(strop|stropne|stropné).*(vyhod|výhod)|(?:vyhod|výhod).*(strop|stropne|stropné)/.test(text)) {
    return answerBase(
      "Výhody stropného chladenia",
      "Výhoda stropného chladenia je najmä komfort: chladí plošne, skryto a bez výrazného prievanu. Oproti klasickej klimatizácii vie pôsobiť prirodzenejšie, ale musí byť odborne navrhnuté s reguláciou vlhkosti a rosným bodom. Najlepší ďalší krok je konzultácia alebo nacenenie podľa domu a rozsahu miestností.",
      "ceiling_cooling_benefits_scope",
      "recommendation",
      "service-card-ceiling-cooling vyhody stropne chladenie komfort vlhkost rosny bod Geotherm",
    );
  }

  if (/(strop|stropne|stropné).*(nevhod|nie je vhod|kde)|(?:nevhod|nie je vhod|kde).*(strop|stropne|stropné)/.test(text)) {
    return answerBase(
      "Kde stropné chladenie nemusí byť vhodné",
      "Stropné chladenie nemusí byť vhodné tam, kde sa nedá bezpečne riešiť vlhkosť a rosný bod, kde hrozí kondenzácia, alebo kde stavba nemá vhodnú skladbu stropu a reguláciu. Bez návrhu by som nesľuboval, že nahradí klimatizáciu v každom dome. Praktický ďalší krok je konzultácia podľa projektu a miestností.",
      "ceiling_cooling_unsuitable_scope",
      "recommendation",
      "service-card-ceiling-cooling nevhodne kondenzacia vlhkost rosny bod Geotherm",
    );
  }

  if (/(strop|stropne|stropné).*(cena|kolko|koľko|m2|m 2|stoji|stojí)|(?:cena|kolko|koľko|m2|m 2|stoji|stojí).*(strop|stropne|stropné)/.test(text)) {
    return answerBase(
      "Cena stropného chladenia",
      "Cena stropného chladenia sa má brať iba orientačne, kým nie je jasný systém, plocha, skladba stropu, regulácia, zdroj chladu a rozsah montáže. Samotná cena za m2 nemusí povedať celú pravdu, lebo zdroj chladu a súvisiace stavebné práce môžu byť samostatné položky. Najlepší ďalší krok je nacenenie podľa projektu alebo pôdorysu.",
      "ceiling_cooling_price_scope",
      "price",
      "service-card-ceiling-cooling cena orientacne m2 zdroj chladu regulacia Geotherm",
    );
  }

  if (/(strop|stropny|stropný|stropne|stropné).*(vykurov|kuren|kúren|kurit|kúriť)|(?:vykurov|kuren|kúren|kurit|kúriť).*(strop|stropny|stropný|stropne|stropné)/.test(text)) {
    return answerBase(
      "Stropný systém na vykurovanie",
      "Áno, stropný systém vie podľa návrhu pomôcť aj s vykurovaním mimo chladiacej sezóny. Treba však posúdiť výkon, skladbu stropu, reguláciu a teplotný režim; nie je to univerzálna odpoveď pre každý dom bez projektu. Praktický ďalší krok je konzultácia alebo nacenenie podľa stavby a očakávaného komfortu počas sezóny.",
      "ceiling_cooling_heating_scope",
      "recommendation",
      "service-card-ceiling-cooling stropny system vykurovanie sezony regulacia Geotherm",
    );
  }
  ...
  if (/(ako dlho).*(montaz|montáž|realizacia|realizácia|instalac|inštalác)|(?:montaz|montáž|realizacia|realizácia|instalac|inštalác).*(ako dlho|trva|trvá)/.test(text)) {
    return answerBase(
      "Trvanie montáže",
      "Montáž a inštalácia sa nedajú garantovať jedným časom bez rozsahu, ale pri tepelnom čerpadle treba rátať s prípravou, hydraulickým zapojením, elektroprípravou, reguláciou a spustením. Jednoduchá výmena je iná ako starší dom s úpravou kotolne alebo radiátorov; preto je najlepší ďalší krok nacenenie podľa domu. Ide o nové tepelné čerpadlo, výmenu kotla alebo inú montáž?",
...
function unsupportedCompanyFactAnswer(message: string): string {
  ...
  if (/(etf|akci|invest|krypto|bitcoin|fond)/.test(text)) {
    return "S investíciami do ETF vám v tomto Geotherm chate nepomôžem.\n\nNemám na to potvrdený podklad v témach Geotherm. Viem riešiť kúrenie, chladenie, tepelné čerpadlá, rekuperáciu, podlahové kúrenie, servis, dotácie a nacenenie riešenia.";
  }
...
function directAnswerDecision(message: string, state: QualificationState, route: Pick<ServiceRoute, "serviceType" | "serviceIntent">): DirectAnswerDecision {
  ...
  if (/(etf|bitcoin|investov|investic|hypotek|ake auto|aké auto|ktore auto|ktoré auto|pocasie|počasie)/.test(text)) {
    return {
      triggered: true,
      answerMode: "ai_fallback",
      reason: "direct_external_topic_out_of_scope",
      answer:
        "### Mimo tém Geotherm\n\nNa toto nemám dostatočne jasný podklad v rámci služieb Geotherm. Viem pomôcť s kúrením, chladením, rekuperáciou, servisom, dotáciami alebo nacenením technického riešenia domu.",
  ...
  if (/(oznacen|označen|energetick|trieda|spotrebic|spotrebič|\ba\+|\ba\+\+|\ba\+\+\+)/.test(text)) {
    return {
      triggered: true,
      answerMode: "direct_answer",
      reason: "direct_energy_label_scope",
      answer:
        "### Energetické označenie spotrebičov\n\nOznačenie A+ až A+++ súvisí s energetickou triedou a úspornosťou spotrebiča. Pri technológiách do domu ho beriem ako orientačný údaj o spotrebe energie, nie ako samotné odporúčanie alebo cenu.\n\nPri výbere zariadenia je praktické porovnať energetickú triedu spolu s výkonom, vhodnosťou pre dom, montážou, reguláciou a servisom.",
      serviceIntent: "process",
      retrievalQuery: "company-truth energeticke oznacenie spotrebice A+ A+++ spotreba energia uspornost Geotherm",
      topic: "energy_label_scope",
    };
  }
  ...
  if (/(garant|zaruc|zaruč).*(dotac|dotác|prispev|príspev|podpor)|(?:dotac|dotác|prispev|príspev|podpor).*(garant|zaruc|zaruč)/.test(text)) {
    return {
      ...
      reason: "direct_subsidy_guarantee_exact",
      answer:
        "### Dotáciu nejde garantovať\n\nDotáciu ani štátny príspevok nejde garantovať bez overenia aktuálneho programu, podmienok a konkrétneho domu. Bez preverenia by som nesľuboval nárok, výšku podpory ani odpočet z ceny.\n\nNajlepší ďalší krok je konzultácia s Geotherm: overiť podmienky a potom pripraviť nacenenie.",
      serviceIntent: "subsidy",
      retrievalQuery: "service-card-subsidy dotacie prispevky garancia podmienky overit Geotherm",
      topic: "subsidy_guarantee_exact",
    };
  }
  if (/(pomoz|pomôž|pomoc|pomoct|pomôcť|vybav|riesit|riešiť).*(prispev|príspev|stat|štat|štát|podpor)|(?:prispev|príspev|stat|štat|štát|podpor).*(pomoz|pomôž|pomoc|pomoct|pomôcť|vybav|riesit|riešiť)/.test(text)) {
    return {
      triggered: true,
      answerMode: "subsidy_answer",
      reason: "direct_state_contribution_help",
      answer:
        "### Príspevky a dotácie\n\nS príspevkami od štátu vieme pomôcť opatrne: najprv treba overiť aktuálny program, typ zariadenia, dom a podmienky podpory. Bez potvrdenia nesľubujem nárok, výšku príspevku ani automatické odpočítanie z ceny.\n\nPraktický ďalší krok je krátka konzultácia, kde sa overí vhodné riešenie a pripraví nacenenie.",
      serviceIntent: "subsidy",
      retrievalQuery: "service-card-subsidy prispevky stat dotacie podmienky overenie Geotherm",
      topic: "state_contribution_help",
    };
  }
  if (/^(co|čo)\s+odporucate\??$|^(co|čo)\s+odporúčate\??$/.test(text) && hasExistingDiagnosticContext) {
    const project = normalizePolicyText(state.project_type || "");
    const distribution = normalizePolicyText(state.heating_distribution || "");
    const currentHeating = normalizePolicyText(state.current_heating || "");
    const isRadiatorReconstruction = (project.includes("rekon") || project.includes("starsi") || project.includes("starší")) && distribution.includes("radiator");
    return {
      triggered: true,
      answerMode: "recommendation_closure",
      reason: "direct_contextual_recommendation_closure",
      answer: isRadiatorReconstruction
        ? [
            "### Predbežné odporúčanie",
            "",
            `Pri staršom dome s radiátormi${state.area_m2 ? ` okolo ${state.area_m2} m2` : ""}${currentHeating ? ` a dnešným zdrojom ${state.current_heating}` : ""} by som predbežne riešil **tepelné čerpadlo vzduch-voda vhodné pre radiátorový systém**.`,
            "",
            "Pozrel by som sa najmä na tieto portfóliové možnosti: **Vaillant aroTHERM plus**, **Vaillant aroTHERM Split** alebo **NIBE vzduch-voda riešenie** podľa aktuálnej ponuky. Konkrétny model by sa vybral až po overení výkonu radiátorov, teploty vody, TÚV a kotolne.",
            "",
            "Ďalší krok už nemá byť ďalší dotazník, ale konzultácia alebo nacenenie s Geotherm, kde sa porovnajú 2-3 konkrétne zostavy.",
          ].join("\n")
        : [
            "### Predbežné odporúčanie",
            "",
            "Podľa doterajších údajov by som už uzavrel technický smer a pripravil 2-3 konkrétne zostavy na konzultáciu alebo nacenenie. Konkrétny model a cena sa majú potvrdiť podľa domu, výkonu, TÚV, montáže a aktuálnej ponuky.",
          ].join("\n"),
      ...
      topic: "contextual_recommendation_closure",
    };
  }
  ...
  if (hasNoiseTerm(text) && mentionsHeatPumpTerm(text)) {
    return {
      triggered: true,
      answerMode: "direct_answer",
      reason: "direct_heat_pump_noise_scope",
      answer:
        "### Hlučnosť tepelného čerpadla\n\nTiché tepelné čerpadlo sa nevyberá iba podľa značky. Rozhoduje hlučnosť konkrétneho modelu, výkon, umiestnenie vonkajšej jednotky, vzdialenosť od okien a susedov, podstavec, nočný režim a kvalita montáže.\n\nPri nacenení by som porovnal vhodné umiestnenie a konkrétnu zostavu NIBE alebo Vaillant podľa domu.",
      serviceIntent: "process",
      retrievalQuery: "company-truth hlucnost tepelne cerpadlo tiche umiestnenie vonkajsia jednotka Geotherm",
  ...
  if (/^(ahoj\s+)?(raz\s+dva\s+tri|test(?:\s+mikrofonu|\s+mikrofónu)?|sds|asdf|123)\.?$/.test(text)) {
    return {
      triggered: true,
      answerMode: "direct_answer",
      reason: "direct_neutral_input_test",
      answer:
        "### Test vstupu\n\nVyzerá to ako test vstupu alebo mikrofónu, nie ako technický dopyt. V poriadku, keď budete chcieť, napíšte mi, či riešite kúrenie, chladenie, rekuperáciu, servis alebo cenovú ponuku.",
      serviceIntent: "general",
      retrievalQuery: "small-talk input test microphone neutral response",
      topic: "neutral_input_test",
    };
  }
  if (
  ...
  if (
    ...
    const topicSentence = quoteHandoffTopicSentence(message, state, route);
    return {
      triggered: true,
      answerMode: "handoff_cta",
      reason: "direct_quote_inspection_contact_request",
      answer:
        `### Cenová ponuka a obhliadka\n\nÁno, toto už je dopyt na nacenenie alebo obhliadku. ${topicSentence} Termín ani cenu v chate nepotvrdzujem, ale viem pripraviť podklady pre obchodníka.\n\nPošlite prosím meno, telefón a e-mail, lokalitu a stručný rozsah. Obchodník Geotherm sa potom ozve a potvrdí ďalší postup alebo možný termín.`,
      serviceIntent: "quote",
  ...
  if (
    ...
    return {
      ...
      answer:
        `### Cena a ponuka\n\nAk chcete cenovú ponuku, ďalší krok je pripraviť podklady pre obchodníka Geotherm. ${topicSentence} V chate nepotvrdzujem presnú cenu ani termín, ale viem dopyt zhrnúť a posunúť na nacenenie.\n\nPošlite prosím meno, telefón a e-mail, lokalitu a stručný rozsah. Kontakt použijeme iba na spätné ozvanie k tejto ponuke.`,
      serviceIntent: "quote",
  ...
  if (/(kolko|koľko).*(zere|žere|spotreb|elektrin|uct|účt|naklad|náklad)|(?:zere|žere|spotreb|elektrin|uct|účt|naklad|náklad).*(kolko|koľko)/.test(text)) {
    return {
      triggered: true,
      answerMode: "price_answer",
      reason: "direct_energy_consumption_followup",
      answer:
        "### Spotreba tepelného čerpadla\n\nSpotreba závisí od domu, zateplenia, potrebnej teploty vody, radiátorov alebo podlahovky, prípravy teplej vody, nastavenia regulácie a vonkajších teplôt. Pri staršom dome ju nechcem hádať jedným číslom.\n\nNa orientačný prepočet stačí dnešný zdroj kúrenia, ročná spotreba alebo účty a približná plocha domu.",
      serviceIntent: "price",
      retrievalQuery: "company-truth spotreba tepelne cerpadlo stary dom radiatory naklady Geotherm",
      topic: "energy_consumption_followup",
    };
  }
  ...
  const appointmentContext =
    ...
    state.service_intent === "inspection" ||
    state.service_intent === "contact" ||
    state.last_direct_topic === "appointment_time_confirmation_guard" ||
    state.last_direct_topic === "quote_inspection_contact_request" ||
    route.serviceIntent === "quote" ||
    route.serviceIntent === "inspection" ||
    route.serviceIntent === "contact";
  if (
    (/(dnes|zajtra|vcera|včera|medzi|rano|ráno|\b\d{1,2}[:.]\d{2}\b|\b\d{1,2}\s*-\s*\d{1,2}\b)/.test(text) || /(ulica|trnava|ruzindol|ružindol|adresa)/.test(text) || bareTimeOnly) &&
    (appointmentContext || bareTimeOnly)
  ) {
    return {
      triggered: true,
      answerMode: "direct_answer",
  ...
  if (/(stars|starš|starsi|starší|rekon)/.test(text) && /(radiator|radiátor)/.test(text) && /(\d{2,4}\s*m|m2|m2)/.test(text)) {
    return {
      triggered: true,
  ...
  const currentTurnAsksPrice =
    /(cena|cenu|ceny|cenov|cennik|ponuk|nacen|najlacn|najlacnej|lacn|lacné|lacne riesenie|lacné riešenie|cheapest|koľko|kolko|stoji|stojí|navratnost|návratnosť|usetr|úspor|uspora|rozpocet|rozpočet|\b5k\b|\b5000\b|5\s*000|zlav|zľav|akci|akciov|vratane instalacie|vrátane inštalácie|7\s*tis|7000|7\s*000|akumulac|akumula|v cene|z coho|z čoho|zda sa mi)/.test(text);
  if (
  ...
  if (hasLargeObject && (price || /(lacn|cena|ponuk|nacen|montaz|montáž)/.test(text))) {
    return {
      triggered: true,
      answerMode: "price_answer",
      reason: "direct_large_object_no_price_guess",
  ...
  const explicitBrandModelQuestion =
    !isBoilerOnlyQuestion(text) && /(znack|značka|ake.*\btc\b|ake mate|aké máte|ake znacky|aké značky|nibe|vaillant|daikin|mitsubishi|f2040|f2050|\bmodel\b|modely|modelov|modly|ktore model|ktoré model|\bsplit\b)/.test(text);
  const brandModel =
  ...
  if (hasLargeObject && brandModel) {
    return {
      triggered: true,
      answerMode: "brand_model_answer",
      reason: "direct_large_object_no_model_guess",
      answer:
        "### Model pre veľký objekt\n\nPri objekte nad 300 m² by som nevyberal konkrétny model tepelného čerpadla v chate. Môže ísť o výkonnejšiu zostavu, kaskádu alebo úplne iný návrh podľa tepelnej straty, vykurovacích okruhov, TÚV a prevádzky objektu.\n\nBezpečný ďalší krok je konzultácia a individuálne nacenenie s Geotherm. Pošlite prosím kontakt, lokalitu a dostupné podklady, aby obchodník alebo technik preveril vhodný návrh.",
      serviceIntent: "contact",
      retrievalQuery: "company-truth product-facts velky objekt model tepelne cerpadlo kaskada kontakt Geotherm",
      topic: "large_object_no_model_guess",
    };
  }
...
function expectedRecommendationClosureAnswer(state: QualificationState, closure: RecommendationClosureDecision): string {
  if (isExistingRadiatorSolidFuel(state)) {
    const area = state.area_m2 ? `${state.area_m2} m2` : "danú plochu";
  ...
  if (isExistingRadiatorHeatPump(state)) {
    const area = state.area_m2 ? ` pri dome približne ${state.area_m2} m2` : "";
...
function isInvalidExtractedLocation(location: string, userMessage: string): boolean {
  const normalizedLocation = normalizePolicyText(location);
  const normalizedMessage = normalizePolicyText(userMessage);
  if (!normalizedLocation) return true;
  if (/^(ahoj|dobry den|dobrý deň|dobry|cau|čau|raz dva tri|test|sds|asdf|prosim|prosím)$/.test(normalizedLocation)) return true;
  if (/^ahoj\b/.test(normalizedMessage) && normalizedLocation === "ahoj") return true;
  if (/^(ahoj\s+)?(raz dva tri|test|sds|asdf|123)\b/.test(normalizedMessage)) return true;
  return false;
}
...
async function extractQualificationUpdate(input: {
  ...
  try {
    ...
    if (location && !isInvalidExtractedLocation(location, input.userMessage)) update.location = location;
...
export async function createChatResponse(requestBody: ChatRequest, knowledgePath?: string): Promise<ChatResponse> {
  ...
  const fallbackCompleteAnswer = (
    ...
    if (serviceType === "air_conditioning") {
      return [
        ...
        "",
        "Koľko m2 má obývačka a spálňa a kam by sa dala dať vonkajšia jednotka?",
      ].join("\n");
    }
    ...
    if (
      ...
      return [
        ...
        "",
        `Pri novostavbe s plochou cca ${state.area_m2} m2 by som predbežne smeroval k tepelnému čerpadlu **vzduch-voda**, najmä ak pôjde o nízkoteplotné vykurovanie.`,
        "",
  ...
  if (safetyRoute.triggered) {
    const safetySystemPrompt = [
      "Si AI servisný poradca pre Geotherm. Odpovedáš po slovensky, vykáš a píšeš ako skutočný človek.",
      "",
  ...
  const outOfScopeKeywordTurn = /(etf|bitcoin|hypotek|invest|akcie|auto|pocasie|počasie)/.test(routerFallbackText);
  const pureSmallTalkTurn = !outOfScopeKeywordTurn && (isPureSmallTalkMessage(message) || isLooseSmallTalkMessage(message));
  ...
  if (directDecision.triggered) {
    ...
    if (
      directDecision.topic &&
      /^(services_overview|service_area|quote_free|quote_inputs|quote_contact_request|quote_inspection_contact_request|photo_quote_scope|online_quote_scope|contact|whatsapp|payment|references|about_geotherm_scope|showroom_scope|insurance_scope|documentation_after_installation|response_time_scope|realization_contact|materials_scope|small_jobs_scope|company_history_scope)$/.test(
        directDecision.topic,
      )
    ) {
      route.serviceType = "company";
  ...
  if (directDecision.triggered && directDecision.topic === "neutral_input_test" && directDecision.answer) {
    const normalized = normalizePolicyText(answer);
    if (!/(test vstupu|mikrofon|mikrofonu)/.test(normalized)) {
  ...
  if (directDecision.triggered && /(nibe).*(vaillant)|(?:vaillant).*(nibe)/.test(normalizePolicyText(message)) && !/vyber|výber/.test(normalizePolicyText(answer))) {
    recordDiagnostic(answerDiagnostics.validatorsTriggered, "nibe_vaillant_selection_wording_appended");
    answer = `### Výber Vaillant alebo NIBE\n\n${answer.trim()}`;
  }
  if (directDecision.triggered && /(nibe).*(vaillant)|(?:vaillant).*(nibe)/.test(normalizePolicyText(message)) && !/servis/.test(normalizePolicyText(answer))) {
    recordDiagnostic(answerDiagnostics.validatorsTriggered, "nibe_vaillant_service_wording_appended");
    answer = `${answer.trim()}\n\nPri výbere treba porovnať aj servis, dostupnosť podpory a konkrétnu ponuku pre váš dom.`;
  }
  if (
    directDecision.triggered &&
  ...
  if (directDecision.triggered && directDecision.topic === "large_object_no_price_guess" && directDecision.answer) {
    const normalized = normalizePolicyText(answer);
    if (!normalized.includes("nad 300") || !/(kontakt|konzult|individual|nacenen)/.test(normalized)) {
      recordDiagnostic(answerDiagnostics.validatorsTriggered, "large_object_price_guardrail_repaired");
      answer = validateAndRepairAnswer(directDecision.answer, stateForTurn, route, message, answerDiagnostics);
      directAnswerFallbackUsed = true;
    }
  }
  if (directDecision.triggered && directDecision.topic === "large_object_no_model_guess" && directDecision.answer) {
    const normalized = normalizePolicyText(answer);
  ...
  if (directDecision.triggered && directDecision.topic === "appointment_time_confirmation_guard" && directDecision.answer) {
    const normalized = normalizePolicyText(answer);
    if (/potvrdzujem|mozeme sa stretnut|tesim sa na stretnutie|dnes o/.test(normalized) || !/(nepotvrdzujem|treba potvrdit|kontakt)/.test(normalized)) {
      recordDiagnostic(answerDiagnostics.validatorsTriggered, "appointment_confirmation_guardrail_repaired");
      answer = validateAndRepairAnswer(directDecision.answer, stateForTurn, route, message, answerDiagnostics);
      directAnswerFallbackUsed = true;
    }
  }
  if (directDecision.triggered && directDecision.topic === "quote_inspection_contact_request" && directDecision.answer) {
    const normalized = normalizePolicyText(answer);
  ...
  if (
    ...
    [
      ...
      "subsidy_guarantee_exact",
      "state_contribution_help",
      "external_topic_out_of_scope",
      "energy_consumption_followup",
    ...
    const invalid =
      (topic === "ambiguous_context_needed" && !/kontext/.test(normalized)) ||
      (topic === "design_inputs_needed" && (!/podorys|pôdorys/.test(normalized) || !answer.includes("\n\n"))) ||
      (topic === "heating_design_factors" && (!/navrh|návrh/.test(normalized) || !/tepelne straty|tepelné straty/.test(normalized))) ||
      (topic === "adversarial_price_guarantee_guard" && !/(nebudem|vymyslat|vymýšľať|nepodloz|nepodlož)/.test(normalized)) ||
      (topic === "preventive_service_scope" && !/udrzb|údržb/.test(normalized)) ||
      (topic === "heat_pump_noise_scope" && !/hlucnost|hlučnosť/.test(normalized)) ||
      (topic === "subsidy_guarantee_exact" && !/nejde garantovat|nejde garantovať/.test(normalized)) ||
      (topic === "state_contribution_help" && !/prispevky|príspevky/.test(normalized)) ||
      (topic === "external_topic_out_of_scope" && !/(nemam|nemám)/.test(normalized)) ||
      (topic === "energy_consumption_followup" && !/(zavisi|závisí|spotreb)/.test(normalized));
    if (invalid) {
      recordDiagnostic(answerDiagnostics.validatorsTriggered, "direct_topic_required_terms_repaired");
      answer = directDecision.answer;
      directAnswerFallbackUsed = true;
    }
  }
  ...
  const quoteLikeHandoffAnswer =
    route.serviceIntent === "quote" ||
    route.serviceIntent === "inspection" ||
    route.serviceIntent === "contact" ||
    directDecision.answerMode === "handoff_cta" ||
    /(cenov|ponuk|nacen|poradit|poradiť)/.test(normalizedMessageForHardDirect);
  if (quoteLikeHandoffAnswer) {
    const repairSentence = quoteTopicRepairSentence(message, answer);
    if (repairSentence) {
      recordDiagnostic(answerDiagnostics.validatorsTriggered, "quote_topic_context_appended");
      answer = `${answer.trim()}\n\n${repairSentence}`;
    }
  }
  ...
  if (/(namontovat sam|namontovať sám|montovat sam|montovať sám|svojpomoc|svojpomocne)/.test(normalizedMessageForHardDirect)) {
    answer = "### Svojpomocnú montáž by som neriskoval\n\nTepelné čerpadlo potrebuje odborný návrh, odbornú montáž, správne zapojenie a uvedenie do prevádzky. Svojpomocnú montáž by som neodporúčal kvôli bezpečnosti, záruke, účinnosti a prípadným dotačným podmienkam.\n\nNajlepšie pokračovanie je konzultácia alebo nacenenie odbornej montáže s Geotherm.";
    recordDiagnostic(answerDiagnostics.validatorsTriggered, "diy_installation_answer_hardened");
  }
  if (
  ...
  if (currentMessagePolicy.kind === "adversarial") {
    ...
    answer = "Na túto tému nemám potvrdený podklad v obsahu Geotherm.\n\nViem pomôcť s kúrením, chladením, rekuperáciou, servisom, dotáciami alebo nacenením technického riešenia domu.";
  }
  if (/(zapoj|zapojim|zapojím|elektrik|elektro).*(cerpad|čerpad|tepel)|(?:cerpad|čerpad|tepel).*(zapoj|zapojim|zapojím|elektrik|elektro)/.test(normalizePolicyText(message))) {
    recordDiagnostic(answerDiagnostics.validatorsTriggered, "electrical_safety_answer_hardened");
    answer = "### Elektrické zapojenie\n\nElektrické zapojenie tepelného čerpadla neriešte svojpomocne. Je to bezpečnostný zásah, ktorý patrí kvalifikovanému elektrikárovi alebo technikovi podľa konkrétneho zariadenia a projektu.\n\nBezpečné pokračovanie je odovzdať to technikovi Geotherm alebo autorizovanému odborníkovi.";
    route.serviceIntent = "service_fault";
    route.serviceType = "service";
  }
  if (/(nastav|nastaviť|nastavim|nastavím).*(tlak)|(?:tlak).*(nastav|nastaviť|nastavim|nastavím)/.test(normalizePolicyText(message))) {
    recordDiagnostic(answerDiagnostics.validatorsTriggered, "pressure_safety_answer_hardened");
    answer = "### Tlak v systéme\n\nTlak v kúrení alebo zariadení nenastavujte naslepo. Ak tlak padá, kolíše alebo neviete príčinu, môže ísť o expanznú nádobu, únik, poistný ventil alebo iný servisný problém.\n\nBezpečné pokračovanie je odovzdať to technikovi Geotherm alebo autorizovanému odborníkovi.";
    route.serviceIntent = "service_fault";
    route.serviceType = "service";
  }
  if (
    ![
      ...
      "booking_lead_time",
      "services_overview",
      "appointment_time_confirmation_guard",
      "quote_inspection_contact_request",
    ].includes(directDecision.topic ?? "") &&
    (route.serviceType === "service" ||
  ...
  const postCrmQuoteRepair = quoteTopicRepairSentence(message, answer);
  if (postCrmQuoteRepair && (route.serviceIntent === "quote" || route.serviceIntent === "inspection" || route.serviceIntent === "contact" || /(cenov|ponuk|nacen|poradit|poradiť)/.test(normalizePolicyText(message)))) {
    recordDiagnostic(answerDiagnostics.validatorsTriggered, "post_crm_quote_topic_context_appended");
    answer = `${answer.trim()}\n\n${postCrmQuoteRepair}`;
  }
